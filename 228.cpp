...
public:
	...
	void imgEspecular()
	{
		...
		else
		{
			...
			if (!left().empty())   left().imgEspecular();
			if (!right().empty())  right().imgEspecular();
		}
	}
};
...
bintree<char> leerArbol(string const&linea, int &index) {

	char raiz = linea[index];

	if (linea[index] == ' ') { index++;  return{}; }
	else
	{
		index++;
		auto iz = leerArbol(linea, index);
		auto dr = leerArbol(linea, index);
		return{ iz, raiz, dr };
	}
}
...
int main()
{
	...
	{
		indice = 0;
		bintree<char> arbol = leerArbol(linea, indice);

		arbol.imgEspecular();
		vector<char> v = arbol.preorder();
