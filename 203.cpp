...
public:
	...
	void suma(bintree<T> const& arbol2)
	{
		if (arbol2.empty());				//El arbol a sumar es vacío
		else if (empty())
			raiz = arbol2.raiz;
		else								//Ninguno de los árboles es vacío
		{
			raiz->elem += arbol2.root();

			if (this->left().empty())
				this->raiz->left = arbol2.raiz->left;
			else
				this->left().suma(arbol2.left());

			if (this->right().empty())
				this->raiz->right = arbol2.raiz->right;
			else
				this->right().suma(arbol2.right());
		}
	}
...
int main()
{
	...
	while (!arbol1.empty() || !arbol2.empty())
	{
		arbol1.suma(arbol2);

		vector<int> v = arbol1.preorder();
