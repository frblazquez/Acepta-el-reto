...
int main()
{
  ...
  while(cin)
  {
    alto++; ancho++; specialCase=false;                 // Teniendo en cuenta que va de [0,ancho) [0,alto)
    ...
    while(father(arboles,0)!=father(arboles,last) && i>0 && !specialCase)
    {
      ...
      if((rowPos==alto && colPos==ancho)||(rowPos==0 && colPos==0))
        specialCase = true;
    ...
    if(specialCase)
      cout << rowPos << " " << colPos << '\n';
    else if(father(arboles,0) != father(arboles,last))
      cout << "NUNCA SE PUDO\n";
    else
